...
export default function ReservationPage() {
  ...
  const [startDate, setStartDate] = useState<Date | null>(checkIn ? parse(checkIn, "yyyy-MM-dd", new Date()) : null);
  const [endDate, setEndDate] = useState<Date | null>(checkOut ? parse(checkOut, "yyyy-MM-dd", new Date()) : null);
      ...
      <div className="min-h-screen  flex items-center justify-center">
        <div className="bg-zinc-700 rounded-xl p-8 border border-glacier-700 shadow-2xl max-w-md mx-auto">
          <div className="text-center">
            ...
            <button className="px-6 py-3 bg-glacier-600 hover:bg-glacier-700 text-white rounded-lg transition-colors font-medium" onClick={() => router.back()}>
  ...
  const nights = startDate && endDate ? Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) : startDate ? 1 : 0; // Si solo hay fecha de inicio, asumir una noche
  const total =
    nights *
    (isApartment
      ? accommodation?.availableRooms?.reduce((sum, r) => sum + r.price, 0)!
      : selectedRooms.reduce((sum, room) => sum + (accommodation?.availableRooms?.find(r => r.roomConfigId.toString() === room.roomId)?.price || 0) * room.qty, 0));
    ...
    <div className="min-h-screen  text-glacier-50">
      <div className="max-w-7xl mx-auto py-4 sm:py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-glacier-200 mb-2">{dict.CLIENT.RESERVATION.TITLE || "Reservar Alojamiento"}</h1>
          <p className="text-sm sm:text-base text-glacier-300">{dict.CLIENT.RESERVATION.SUBTITLE || "Completa tu reserva paso a paso"}</p>
        </div>
        ...
        {step === Step.Detalles && (
          <div className="bg-zinc-700 rounded-lg shadow-md p-4 sm:p-6 border border-glacier-700">
            <div className="flex items-center mb-4 sm:mb-6">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-glacier-200 border-b-2 border-glacier-500 pb-2">{dict.CLIENT.RESERVATION.TITLES.DETAILS}</h2>
            </div>
              ...
              <div className="mb-6 sm:mb-8 bg-zinc-600 p-4 sm:p-6 rounded-lg border border-glacier-700">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex-1">
                    <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3 text-glacier-200">{accommodation.name}</h3>
              ...
              <div className="col-span-1 md:col-span-2 bg-zinc-600 p-4 sm:p-6 rounded-lg border border-glacier-700 shadow-md">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center">
                    ...
                    <h3 className="font-semibold text-base sm:text-lg text-glacier-200 border-b-2 border-glacier-500 pb-2">{dict.CLIENT.RESERVATION.LABELS.DATES}</h3>
                  </div>
                  {/* Botón toggle para móvil */}
                  <button className="md:hidden text-glacier-400 hover:text-glacier-200 transition-colors" onClick={() => setShowCalendar(!showCalendar)}>
                ...
                {startDate && endDate && (
                  <div className="md:hidden mb-4 bg-zinc-700 rounded-lg p-3 border border-glacier-700">
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="text-center">
                        <div className="text-glacier-400 mb-1">{dict.CLIENT.RESERVATION.LABELS.CHECKIN}</div>
                        <div className="font-semibold text-glacier-100">{format(startDate, "dd/MM/yyyy")}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-glacier-400 mb-1">{dict.CLIENT.RESERVATION.LABELS.CHECKOUT}</div>
                        <div className="font-semibold text-glacier-100">{format(endDate, "dd/MM/yyyy")}</div>
                      </div>
                    </div>
                    {nights > 0 && (
                ...
                {startDate && (
                  <div className="hidden md:block mt-6 bg-zinc-700 rounded-lg p-4 border border-glacier-700">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="text-center">
                        <div className="text-glacier-400 mb-1">{dict.CLIENT.RESERVATION.LABELS.CHECKOUT}</div>
                        <div className="font-semibold text-glacier-100">{endDate ? format(endDate, "dd/MM/yyyy") : "Por seleccionar"}</div>
                      </div>
                    </div>
                    {nights > 0 && (
                    ...
                    {startDate && !endDate && (
                      <div className="text-center mt-3 pt-3 border-t border-glacier-700">
                        <span className="text-glacier-400 text-xs">Haz clic de nuevo en la misma fecha para reservar una sola noche</span>
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <div className="bg-zinc-600 rounded-lg p-3 sm:p-4 border border-glacier-700">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div className="mb-2 sm:mb-0">
                          <h4 className="font-medium text-glacier-100 text-sm sm:text-base">{dict.CLIENT.RESERVATION.LABELS.APARTMENT}</h4>
                          <p className="text-xs sm:text-sm text-glacier-300 mt-1">{dict.CLIENT.RESERVATION.MSG.APARTMENT_FULL}</p>
                        </div>
                        <div className="bg-glacier-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium self-start sm:self-auto">1x</div>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {selectedRooms.map(room => {
                        const info = accommodation?.availableRooms?.find(r => r.roomConfigId.toString() === room.roomId);
                        const typeLabel = info?.roomType || dict.CLIENT.RESERVATION.DEFAULT.ROOM_TYPE;
                        const cap = info?.capacity || 1;
                        const price = info?.price || 0;
                        return (
                          <div key={room.roomId} className="bg-zinc-600 rounded-lg p-3 sm:p-4 border border-glacier-700">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                              <div className="flex-1 mb-2 sm:mb-0">
                                <h4 className="font-medium text-glacier-100 mb-1 sm:mb-2 text-sm sm:text-base">{typeLabel}</h4>
                              ...
                              <div className="bg-glacier-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium self-start sm:self-auto">x{room.qty}</div>
                            </div>
                          </div>
                        );
                      })}
                ...
                {availableDates.length > 0 && (
                  <div className="bg-zinc-700 rounded-lg shadow-md p-4 sm:p-6 border border-glacier-700">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="font-semibold text-base sm:text-lg text-glacier-200 border-b-2 border-glacier-500 pb-2">{dict.CLIENT.RESERVATION.LABELS.AVAILABILITY}</h3>
                    </div>
                    <div className="grid gap-2">
                      {availableDates.slice(0, 3).map((range, index) => (
                        <div key={range.startDate} className="bg-green-500/10 border border-green-500/30 rounded-lg p-2 sm:p-3">
                      ...
                      {availableDates.length > 3 && <div className="text-center text-glacier-400 text-xs sm:text-sm mt-2">+{availableDates.length - 3} periodos más</div>}
                    </div>
                  </div>
                )}
              </div>
        ...
        {step === Step.Pago && (
          <div className="bg-zinc-700 rounded-lg shadow-md p-4 sm:p-6 border border-glacier-700">
            <div className="flex items-center mb-4 sm:mb-6">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-glacier-200 border-b-2 border-glacier-500 pb-2">{dict.CLIENT.RESERVATION.TITLES.PAY}</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              {/* Resumen */}
              <div className="bg-zinc-600 rounded-lg p-4 sm:p-6 border border-glacier-700 shadow-md">
                <h3 className="font-semibold text-base sm:text-xl mb-4 sm:mb-6 text-glacier-200 border-b-2 border-glacier-500 pb-3">{dict.CLIENT.RESERVATION.LABELS.SUMMARY}</h3>
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-start py-2 border-b border-glacier-700">
                    <span className="text-xs sm:text-lg text-glacier-300 font-medium">{dict.CLIENT.RESERVATION.LABELS.ACCOMMODATION}:</span>
                    <span className="font-medium text-xs sm:text-lg text-glacier-100 text-right ml-2">{accommodation?.name || "N/A"}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-glacier-700">
                    <span className="text-xs sm:text-lg text-glacier-300 font-medium">{dict.CLIENT.RESERVATION.LABELS.CHECKIN}:</span>
                    <span className="font-medium text-xs sm:text-lg text-glacier-100">{startDate ? format(startDate, "dd/MM/yyyy") : "-"}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-glacier-700">
                    <span className="text-xs sm:text-lg text-glacier-300 font-medium">{dict.CLIENT.RESERVATION.LABELS.CHECKOUT}:</span>
                    <span className="font-medium text-xs sm:text-lg text-glacier-100">{endDate ? format(endDate, "dd/MM/yyyy") : "-"}</span>
                  </div>
                  <div className="flex justify-between items-center pt-4 text-base sm:text-xl">
                    <span className="font-bold text-glacier-200">{dict.CLIENT.RESERVATION.LABELS.TOTAL}:</span>
                    <span className="font-bold text-2xl sm:text-2xl text-glacier-100">{total}€</span>
                  </div>
                </div>
              </div>
          ...
          return (
            <React.Fragment key={key}>
              <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                ...
                <span className={`text-xs sm:text-sm font-medium whitespace-nowrap ${completed || active ? "text-glacier-100" : "text-glacier-400"}`}>{label}</span>
              </div>
              {i < Object.keys(dict).length - 1 && <div className={`w-6 sm:w-12 h-0.5 transition-colors duration-300 flex-shrink-0 ${completed ? "bg-green-400" : "bg-glacier-700"}`} />}
            </React.Fragment>
          );
        })}
      </div>
