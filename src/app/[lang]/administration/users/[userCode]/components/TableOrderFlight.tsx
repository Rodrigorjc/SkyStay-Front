...
import { OrderFlightVO } from "../types/order";
import { useDictionary } from "@context";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/app/components/ui/admin/Table";

interface TableOrderFlightProps {
  data: OrderFlightVO[];
}

export default function TableOrderFlight({ data }: TableOrderFlightProps) {
  const { dict } = useDictionary();

  return (
    <div>
      <section>
        <h3 className="my-4 text-2xl text-glacier-300">{dict.ADMINISTRATION.USERS.DETAILS.FLIGHT_ORDER}:</h3>
              ...
              {data.length === 0 ? (
                ...
                data.map((d, index) => (
                  <TableRow key={d.order_code} isOdd={index % 2 === 0}>
                    <TableCell>{d.flight_code}</TableCell>
